...
--Cree una tabla llamada agenda. Debe tener los siguiente campos:
--apellido, varchar(30); nombre, 
--varchar(20); domicilio, varchar (30) y telefono, varchar(11):
...
/*10. Recupere los registros que contengan valor "null" 
en el campo "laboratorio", luego los que 
tengan una cadena vacía en el mismo campo.
Note que el resultado es diferente*/
...
/*11. Recupere los registros que contengan valor 
...
/*12. Recupere los registros cuyo laboratorio no contenga 
una cadena vacía, luego los que sean 
distintos de "null".
Note que la salida de la primera sentencia no muestra 
los registros con cadenas vacías y tampoco los 
que tienen valor nulo; el resultado de la segunda sentencia 
muestra los registros con valor para el 
...
para almacenar los datos descritos arriba:

 - Número de cuenta: entero, no nulo, no puede haber 
 valores repetidos, clave primaria;
 - Documento del propietario de la cuenta: cadena de 
 caracteres de 8 de longitud (siempre 8), no nulo;
 - Nombre del propietario de la cuenta: cadena de 
 caracteres de 30 de longitud,
...
/*5- Cree una regla para restringir los valores que s
e pueden ingresar en un campo "patente" (3 letras 
seguidas de 3 dígitos):*/

 create rule RG_patente_patron
 as @patente like '[A-Z][A-Z][A-Z][0-9][0-9][0-9]';

/*6- Ejecute el procedimiento almacenado del
sistema "sp_help" para ver que la regla creada 
anteriormente existe:*/
 exec sp_help;

/*7- Ejecute el procedimiento almacenado del sistema 
"sp_helpconstraint" para ver que la regla creada 
anteriormente no está asociada aún a ningún campo
de la tabla "vehiculos".*/
 exec sp_helpconstraint vehiculos;
 exec sp_bindrule RG_patente_patron,'vehiculos.patente';

/*8-  Asocie la regla al campo "patente":
Note que hay una patente que no cumple la regla, 
SQL Server NO controla los datos existentes, pero 
si controla las inserciones y actualizaciones:*/
 select * from empleados;

/*9- Intente ingresar un registro con valor
para el campo "patente" que no cumpla con la regla.
aparece un mensaje de error indicando que 
hay conflicto con la regla y la inserción no se realiza.*/
...
/*14- Si la asociamos a un campo que ya tiene 
asociada otra regla, la nueva regla reeemplaza la 
asociación anterior. Asocie la regla creada en el
punto anterior al campo "tipo".*/
...
/*19- Intente establecer una restricción "check" 
que asegure que la fecha y hora de llegada a la playa 
...
/*22- Cree una restricción "default" que almacene el valor "b" en el campo "tipo:
 ...
 default 'b'
 for tipo;
/*Note que esta restricción va contra la regla 
asociada al campo "tipo" que solamente permite los 
valores "a", "c" y "m". SQL Server no informa el 
conflicto hasta que no intenta ingresar el valor 
por defecto.*/
...
/*24- Vea las reglas asociadas a "empleados" y las restricciones
aplicadas a la misma tabla ejecutando 
"sp_helpconstraint".
Muestra 1 restricción "check", 1 restricción
"default" y 4 reglas asociadas.*/
 exec sp_helpconstraint vehiculos;
...
"VP_datodesconocido"al campo "fechaingreso".
Note que se asoció un valor predeterminado
de tipo caracter a un campo de tipo "datetime"; SQL 
Server lo permite, pero al intentar ingresar
el valor aparece un mensaje de error.*/
...
No lo permite porque son de distintos tipos.*/
 insert into clientes default values;
--18- Cree una regla que entre en conflicto con 
--el valor predeterminado "VP_legajo_patron".
 create rule RG_legajo
  as @valor like 'B%';
/*19- Asocie la regla al campo "legajo".
Note que la regla especifica que el campo "legajo"
debe comenzar con la letra "B", pero el valor 
predeterminado tiene el valor "AA00"; SQL Server 
realiza la asociación, pero al intentar ingresar el 
valor predeterminado, no puede hacerlo y muestra 
un mensaje de error.*/
...
/*2- Recuerde que si elimina una tabla, las
asociaciones de reglas y valores predeterminados de sus 
campos desaparecen, pero las reglas y valores 
predeterminados siguen existiendo. Si intenta crear 
una regla o un valor predeterminado con igual nombre
que uno existente, aparecerá un mensaje 
indicándolo, por ello, debe eliminarlos (si existen)
...
--3Registros duplicados (distinct)
--Una empresa tiene registrados sus clientes en una tabla llamada "clientes".
--1- Elimine la tabla "clientes", si existe:
 if object_id('clientes') is not null
  drop table clientes;
...
1- Elimine la tabla, si existe, créela y cargue un registro:*/
 ...
 insert into empleados(apellido,nombre) values ('Castillo','Jeremy');

 --2- Agregue el campo "sueldo", de tipo decimal(5,2).
  insert into empleados(apellido,nombre) values ('Taveras','\Adonis');
   alter table empleados
